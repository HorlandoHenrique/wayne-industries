...
def init_db():
    ...
    # ── BATCAVERNA · SISTEMAS ────────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS bat_sistemas (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        nome TEXT NOT NULL, categoria TEXT NOT NULL DEFAULT 'Computação',
        status TEXT NOT NULL DEFAULT 'operacional', localizacao TEXT, uptime TEXT,
        origem TEXT, observacoes TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO bat_sistemas (codigo,nome,categoria,status,localizacao,uptime,origem,observacoes) VALUES (?,?,?,?,?,?,?,?)', [
        ('SYS-001','Bat-Computador Central','Computação','operacional','Nível B2 · Sala Principal','99.97%','Wayne Enterprises R&D','Processamento quântico, 12 monitores holográficos'),
        ('SYS-002','Reator de Fusão WE-7','Energia','operacional','Nível B5 · Câmara de Energia','100%','Wayne Enterprises R&D','Energia suficiente para 72h de operação autônoma'),
        ('SYS-003','Rede de Câmeras Externas','Vigilância','operacional','Perímetro externo','98.1%','WayneTech','52 câmeras de alta resolução, visão noturna'),
        ('SYS-004','Sistema de Comunicação Cripto','Comunicação','operacional','Nível B1 · Torre de Antenas','99.99%','Fox Laboratories','Criptografia quântica AES-512'),
        ('SYS-005','Garagem Automatizada','Transporte','operacional','Nível B3','100%','Wayne Enterprises','Plataforma giratória, elevadores de alta velocidade'),
        ('SYS-006','Defesas de Perímetro','Defesa','standby','Todos os acessos','—','Wayne Enterprises Military','Portões blindados, grade de contenção EMP'),
        ('SYS-007','Controle Ambiental','Ambiental','operacional','Nível B1 ao B6','100%','Lucius Fox','Temperatura, umidade e pressão reguladas'),
        ('SYS-008','Servidor de Backup Quântico','Computação','standby','Nível B4 · Sala Fria','—','Fox Laboratories','Failsafe com 3 redundâncias geográficas'),
    ])

    # ── BATCAVERNA · TROFÉUS ──────────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS bat_trofeus (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        nome TEXT NOT NULL, categoria TEXT NOT NULL DEFAULT 'Vilão',
        status TEXT NOT NULL DEFAULT 'exposicao', origem TEXT, local_exibicao TEXT,
        primeiro_aparecimento TEXT, observacoes TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO bat_trofeus (codigo,nome,categoria,status,origem,local_exibicao,primeiro_aparecimento,observacoes) VALUES (?,?,?,?,?,?,?,?)', [
        ('TRF-001','T-Rex Mecânico','Histórico','exposicao','Penny Plunderer','Salão Principal · Nível A','Batman #35 (1946)','Réplica em tamanho real de dinossauro robótico'),
        ('TRF-002','Moeda Dupla Face de Harvey Dent','Vilão','restrito','Duas-Caras','Cofre B · Nível B1','Detective Comics #66 (1942)','Moeda original usada por Harvey Dent. Acesso restrito.'),
        ('TRF-003','Cartas do Coringa','Vilão','exposicao','Coringa','Vitrine Central · Nível A','Batman #1 (1940)','Coleção completa de 52 cartas de baralho modificadas'),
        ('TRF-004','Máscara do Bane','Vilão','armazenado','Bane','Depósito · Nível B2','Batman: Vengeance of Bane (1993)','Máscara original do sistema de Venom de Bane'),
        ('TRF-005','Chapéu Cartola do Charada','Vilão','exposicao','Charada','Vitrine 3 · Nível A','Detective Comics #140 (1948)','Com ponto de interrogação original gravado à mão'),
    ])

    # ── BATCAVERNA · LABORATÓRIO ──────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS bat_laboratorio (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        nome TEXT NOT NULL, categoria TEXT NOT NULL DEFAULT 'Forense',
        status TEXT NOT NULL DEFAULT 'ativo', prioridade TEXT DEFAULT 'MÉDIA',
        amostra TEXT, responsavel TEXT, prazo TEXT, localizacao TEXT, observacoes TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO bat_laboratorio (codigo,nome,categoria,status,prioridade,amostra,responsavel,prazo,localizacao,observacoes) VALUES (?,?,?,?,?,?,?,?,?,?)', [
        ('LAB-001','Análise de Toxina · Hera Venenosa','Químico','ativo','ALTA','Amostra vegetal #12-C','Lucius Fox','8h restantes','Bancada A · Setor 1','Composto orgânico desconhecido — risco biológico nível 2'),
        ('LAB-002','DNA Anônimo · Cena do Crime','Forense','analise','MÁXIMA','Amostra sanguínea #47','Alfred Pennyworth','2h restantes','Bancada B · Setor 3','Ligado ao caso Falcone — prioridade máxima'),
        ('LAB-003','Fragmento de Batarang','Forense','concluido','BAIXA','Metal #03-F','Bruce Wayne','—','Bancada C','Liga de tungstênio e carbono — composição confirmada'),
        ('LAB-004','Substância Joker Venom','Químico','ativo','MÁXIMA','Composto químico #X','Lucius Fox','12h restantes','Câmara isolada · B3','Altamente volátil — protocolo de contenção ativo'),
    ])

    # ── BATCAVERNA · SEGURANÇA ────────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS bat_seguranca (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        nome TEXT NOT NULL, categoria TEXT NOT NULL DEFAULT 'Perímetro',
        status TEXT NOT NULL DEFAULT 'armado', nivel_ameaca TEXT,
        cobertura TEXT, ultima_ativacao TEXT, observacoes TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO bat_seguranca (codigo,nome,categoria,status,nivel_ameaca,cobertura,ultima_ativacao,observacoes) VALUES (?,?,?,?,?,?,?,?)', [
        ('SEC-001','Protocolo Alfa · Acesso Manor','Acesso','armado','3','Entrada principal e alas','—','Verificação biométrica tripla — retina, voz, impressão'),
        ('SEC-002','Grade EMP de Perímetro','Perímetro','armado','8','Todo o perímetro externo','Nunca','Desativa qualquer eletrônico no raio de 50m'),
        ('SEC-003','Câmeras Internas','Vigilância','armado','2','Todos os níveis B1–B6','—','47 câmeras ativas com IA de reconhecimento facial'),
        ('SEC-004','Protocolo Sombra · ÔMEGA','Protocolo','desarmado','10','Cidade de Gotham','—','Contra-medidas para meta-humanos hostis — ativação apenas pelo Master'),
        ('SEC-005','Portões de Contenção Blindados','Perímetro','armado','7','Acessos B3 e B4','02/01 · 03:12','Resistência a explosivos convencionais confirmada'),
    ])

    # ── GOTHAM · DISTRITOS ────────────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS gotham_distritos (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        nome TEXT NOT NULL, categoria TEXT NOT NULL DEFAULT 'Ilha Principal',
        status TEXT NOT NULL DEFAULT 'monitorado', populacao TEXT, nivel_crime TEXT,
        controle_atual TEXT, observacoes TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO gotham_distritos (codigo,nome,categoria,status,populacao,nivel_crime,controle_atual,observacoes) VALUES (?,?,?,?,?,?,?,?)', [
        ('GTH-001','The Bowery','Ilha Principal','alerta','85.000','8','Crime Organizado / GCPD','Alta densidade criminal. Refúgio de vilões de baixo escalão.'),
        ('GTH-002','Gotham Heights','Ilha Principal','seguro','42.000','2','GCPD / Batman','Bairro nobre. Residência de Bruce Wayne e outras elites.'),
        ('GTH-003','Crime Alley','Ilha Principal','critico','12.000','10','Sem controle definido','Local do assassinato dos Waynes. Extremamente perigoso.'),
        ('GTH-004','Arkham Island','Ilha Sul','monitorado','800','6','GCPD / Arkham Staff','Ilha do Asilo Arkham. Fugas frequentes registradas.'),
        ('GTH-005','Diamond District','Ilha Norte','monitorado','55.000','5','GCPD','Distrito comercial de luxo. Alvo recorrente de Selina Kyle.'),
        ('GTH-006','Tricorner Island','Ilha Sul','alerta','31.000','7','Falcone Crime Family','Área portuária dominada pelo crime organizado.'),
    ])

    # ── GOTHAM · OCORRÊNCIAS ──────────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS gotham_ocorrencias (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        categoria TEXT NOT NULL DEFAULT 'Confronto',
        status TEXT NOT NULL DEFAULT 'ativa', prioridade TEXT DEFAULT 'MÉDIA',
        setor TEXT, suspeito TEXT, vitimas TEXT DEFAULT '0',
        data_hora TEXT, descricao TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO gotham_ocorrencias (codigo,categoria,status,prioridade,setor,suspeito,vitimas,data_hora,descricao) VALUES (?,?,?,?,?,?,?,?,?)', [
        ('OC-0001','Terrorismo','ativa','MÁXIMA','Gotham Bridge','Coringa','0','20/04 · 23:58','Dispositivo explosivo detectado. Batman em campo.'),
        ('OC-0002','Roubo','resolvida','ALTA','Diamond District','Catwoman','0','18/04 · 02:30','Joias da exposição Wayne roubadas. Suspeita capturada.'),
        ('OC-0003','Fuga','ativa','ALTA','Arkham Asylum','Charada','0','19/04 · 04:15','Fuga durante manutenção. Localização desconhecida.'),
        ('OC-0004','Confronto','investigando','MÉDIA','The Bowery','Desconhecido','3','17/04 · 21:00','Tiroteio entre facções rivais. GCPD no local.'),
        ('OC-0005','Químico','pendente','MÁXIMA','Tricorner Island','Hera Venenosa','0','15/04 · 18:45','Liberação de gás tóxico no porto. Área evacuada.'),
    ])

    # ── GOTHAM · SUSPEITOS ────────────────────────────────
    c.execute('''CREATE TABLE IF NOT EXISTS gotham_suspeitos (
        id INTEGER PRIMARY KEY AUTOINCREMENT, codigo TEXT NOT NULL UNIQUE,
        nome TEXT NOT NULL, categoria TEXT NOT NULL DEFAULT 'Meta-Humano',
        status TEXT NOT NULL DEFAULT 'solto', territorio TEXT,
        nivel_ameaca TEXT, ultima_localizacao TEXT, observacoes TEXT,
        criado_em DATETIME DEFAULT CURRENT_TIMESTAMP,
        atualizado_em DATETIME DEFAULT CURRENT_TIMESTAMP)''')
    c.executemany('INSERT OR IGNORE INTO gotham_suspeitos (codigo,nome,categoria,status,territorio,nivel_ameaca,ultima_localizacao,observacoes) VALUES (?,?,?,?,?,?,?,?)', [
        ('SUS-001','Coringa / Jack Napier','Meta-Humano','solto','Sem território fixo','10','Gotham Bridge · 23:58 de hoje','Prioridade máxima. Imprevisível. Não negociar.'),
        ('SUS-002','Selina Kyle / Catwoman','Ladrão','solto','Diamond District','5','Diamond District · 18/04','Cooperação ocasional com Batman. Monitorar.'),
        ('SUS-003','Edward Nygma / Charada','Crime Organizado','foragido','Gotham Central','7','Arkham · fuga em 19/04','Em fuga desde a madrugada. Rastrear sinais de charadas.'),
        ('SUS-004','Harvey Dent / Duas-Caras','Assassino','internado','—','8','Arkham Asylum','Internado no Ala B. Avaliação psiquiátrica em andamento.'),
        ('SUS-005','Pamela Isley / Hera Venenosa','Cientista','foragido','Tricorner / Áreas verdes','8','Porto de Gotham · 15/04','Responsável pelo incidente químico em Tricorner.'),
        ('SUS-006','Oswald Cobblepot / Pinguim','Crime Organizado','solto','Iceberg Lounge','7','Iceberg Lounge · The Bowery','Operações através de fachadas legais. Difícil de prender.'),
    ])
